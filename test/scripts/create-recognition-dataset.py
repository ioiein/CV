...
def get_crop(image, box):
    x_min = np.clip(min(box[:, 0]) - 3, 0, image.shape[1])
    x_max = np.clip(max(box[:, 0]) + 3, 0, image.shape[1])
    y_min = np.clip(min(box[:, 1]) - 3, 0, image.shape[0])
    y_max = np.clip(max(box[:, 1]) + 3, 0, image.shape[0])
